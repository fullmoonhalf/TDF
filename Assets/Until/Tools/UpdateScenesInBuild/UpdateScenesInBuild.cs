...
namespace until.tool.singleton
{
    public class UpdateScenesInBuild
    {
        ...
        static void execute()
        {
            ...
            Log.info(nameof(UpdateScenesInBuild), "collect SceneEntryHolder scenes.");
            foreach (var asset_guid in assets)
            {
                ...
                if (asset == null)
                {
                    Log.error(nameof(UpdateScenesInBuild), $"{asset_path} is not found.");
                }
            ...
            Log.info(nameof(UpdateScenesInBuild), "generate script.");
            ...
            Log.info(nameof(UpdateScenesInBuild), "done.");
        }
    }
}
